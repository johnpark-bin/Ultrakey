...
⚠️ **갱신(2026-09-01, 이슈 #65 Phase 1 진단) — 이 검증은 "멈추지 않는다"만 확인했지 "리매핑이
다시 동작한다"는 확인하지 못했다.** 위 로그의 `23:59:20` 줄을 보면 `Granted` 전이 후
`hide_modal` 만 찍히고 **탭 재생성/`Active` 전이 로그가 한 줄도 없다** — 당시 코드
(`start_engine_if_needed` 가 `state.engine` 이 `Some` 이면 즉시 `return`)에서는 이미 만들어진
(내부 탭은 죽은) `Engine` 이 슬롯을 계속 차지해, 권한을 재부여해도 앱을 재시작하기 전까지
리매핑이 영구히 복귀하지 않는 별개의 결함이 있었다. Phase 2(이슈 #65)가 `on_permission_transition`
의 `Denied`/`OutOfSync`/`Unknown` 분기에서 `Engine::shutdown()` 을 호출해 슬롯을 비우도록 고쳤다
— 아래 신규 항목이 "재시작 없이 실제로 복귀하는가"를 명시적으로 검증한다.

---

### 1-e. ⭐ 신규(이슈 #65 Phase 2) — 권한 회수 후 방치 → 재부여 → 재시작 없이 리매핑 복귀

위 1-c/1-d 는 권한을 **곧바로**(수 초~35초 안에) 다시 켰다. 이 항목은 훨씬 더 길게 방치해
(a) 트램폴린의 연속 재활성화 예산이 확실히 소진되고 탭이 해체되며, (b) `state.engine` 슬롯이
비워지는지, (c) 그 뒤 권한을 재부여했을 때 **앱을 재시작하지 않고도** 완전히 새 탭이 만들어져
리매핑이 실제로 동작하는지까지 확인한다.

절차:

```
1) 서명된 .app 을 `open` 으로 실행하고, 리매핑 규칙이 하나 이상 켜져 있는지 확인한다
   (예: `Remap caps lock to: left control`).
2) 리매핑이 정상 동작하는지 먼저 확인한다(caps lock → control).
3) 시스템 설정 ▸ 개인정보 보호 및 보안 ▸ 손쉬운 사용에서 Ultrakey 를 끈다.
4) ⭐ 최소 1분 이상 방치한다(1-c 의 "곧바로 다시 켜기"보다 훨씬 길게 — 배경 폴링 주기
   5초를 여러 번 지나치게 하기 위함이다).
5) 같은 화면에서 Ultrakey 를 다시 켠다.
6) 재시작 없이 caps lock → control 이 다시 동작하는지 확인한다.
7) `~/Library/Logs/Ultrakey/ultrakey.log` 를 확인한다.
```

기대(로그):

- 회수 직후: `탭이 해체됐다`/`tearing down the tap` 류 경고 1회, 곧이어 `탭 상태 전이 s=NotInstalled`.
  **반복되는 `재활성화`/`비활성화` 핑퐁이 없어야 한다**(초당 여러 번 찍히면 실패 — 회귀).
- `engine stopped after permission loss` — `state.engine` 슬롯이 실제로 비었다는 증거.
- 재부여 후: `engine started` 와 함께 **`탭 상태 전이 s=Active` 가 다시 찍혀야 한다** — 이게
  이 항목의 핵심 통과 기준이다. 이 줄이 없으면(예: `hide_modal` 만 찍히고 끝) 재발이다.

통과 기준: 6번에서 실제로 caps lock 리매핑이 동작하고, 7번 로그에 `Active` 재전이가 보인다.

⬜ **위임 세션은 이 항목을 수행하지 않는다** — ⛔ 재현 금지 제약(앱을 띄워 권한을 해제하면 안
된다)이 정확히 이 절차와 충돌한다. 사용자가 직접 수행해 이 항목을 닫아야 한다.
...
| 2 | 정보 표시 | ⭐ 이슈 #96 레이아웃 — 상단 **히어로**(앱 아이콘 · 앱 이름 `Ultrakey` · 버전 `v{0}`) + 정보 행 6종(제작자 `John Park` · 웹사이트·연락·이슈 등록 링크 · 앱 설치 위치 · 로그 파일 위치). 앱 이름·버전은 히어로가 표시하므로 별도 라벨 행이 없다 |
...
| 12 | ⭐ 앱 아이콘(이슈 #96) | 히어로 상단에 앱 아이콘(정본 선화, `currentColor`)이 보인다. **라이트에서는 검은 선화, 다크에서는 흰 선화**로 외관을 따라간다 |
| 13 | ⭐ 창 크기(이슈 #96) | 540×480pt 고정(`resizable(false)` — 드래그로 못 키운다). **5개 언어 어느 것에서도 스크롤바가 나타나지 않고** 본문 전체가 보이며, 본문은 창 세로 중앙에 위치한다 |
| 14 | ⭐ 다크 모드(이슈 #96) | 시스템 외관을 다크로 전환해 About 창을 다시 열면 배경·글자·아이콘·버튼 전부 다크 대비로 읽힌다(`color-scheme: light dark` + 시스템 색) |
...
- [ ] ⭐ **K5(이슈 #73).** `원격 데스크톱 …` 아래에 **`한국어 키 처리를 건너뛸 앱`
      목록 편집기**가 있다. 오버라이드 부재 시 기본 12종(`com.apple.ScreenSharing` …
      `com.devolutions.remotedesktopmanager`)이 행으로 채워져 있다. 행 입력에 번들 ID
      를 쓰고 `앱 추가` 로 행을 늘리고 × 로 지우면 저장된다. `기본 목록으로 되돌리기`
      버튼은 오버라이드가 존재할 때만 보인다(부재 = 기본)
- [ ] ⭐ **K9.** `modifier 키와 함께 누른 문자 키를 영어 소문자로 입력` 항목이
      **기본으로 꺼져 있다** — ⚡ 클론 고유 확장이다(원본에 없다)
...
- **원격 데스크톱 제외 게이트의 실제 앱 동작** — 목록의 클라이언트가 이 기기에 설치돼
  ...
  ⭐ **K5(이슈 #73) 이후 남는 것**: 목록 편집 UI 의 조작(행 추가/삭제/되돌리기)이
  settings.json 에 저장되고 게이트에 즉시 반영되는지는 앱을 띄워 손으로 확인해야 한다.
  절차: `ui.lastTab="korean"` 으로 부팅 → 행 편집 → `settings.json` 의
  `korean.excludedBundleIds` 키가 바뀌는지 → 로그의 게이트 재판정.
- ⭐ **K9(이슈 #73)의 실제 글자 관찰** — `key_poke` 로 `⌥`+`A` 를 흉내 낼 수 있지만
  (`fc:0x3A:0x80020 sleep:80 tap:0x00 sleep:80 fc:0x3A:0x0`), 한국어 입력기가 실제로
  `A` 를 한글로 인식하는지는 **사람이 직접 타이핑**해야 관찰할 수 있다. 절차:
  ① 한국어 입력기 + 옵션 ON → TextEdit 에서 `⌥`+`A` 를 눌러 `a` 가 나오는지,
  ② 옵션 OFF 로 같은 입력 → 기존 동작(한글/기호)이 나오는지 대조한다. ⛔ 대조군이
  없으면 "우리 치환의 결과인지"를 증명할 수 없다(§7-3 의 대조군 원칙과 같다).
...
### ⭐ 10-d. 세션 중 한/영 키 전환 (이슈 #76 신설)

> 대상 명세: `docs/spec/seek-activation-and-session.md` §3.1 "한/영 키 예외" · §8 ⭐ 항목.
> 이 항목이 확인하는 것은 **세션이 열려 있는 동안 한/영 키(`0x68`)가 원본 그대로 통과
> 되어 입력 소스가 전환되는가**와 **그 과정이 세션·다른 키의 라우팅을 건드리지 않는가**
> 다. 통과 판정 자체는 단위 테스트(`arbitration.rs` T1~T8)가 덮지만, **입력 소스가
> 실제로 바뀌는가**는 실기기에서만 볼 수 있다.
>
> ⚠️ **한국어 106키 물리 키보드가 없는 환경에서는 아래 실기기 확인이 불가능하다.** 그
> 경우 `ULTRAKEY_TRACE_TAP=1` 로 한/영 키의 도착 형태(`raw_keycode`·`raw_kind`)를
> 기록하는 것까지가 가능한 검증이다(korean-input.md §3.2 절차 4). JIS(일본어) 배열
> 키보드의 `かな` 키는 같은 `0x68` 이므로 물리 위치 도달까지의 절반은 그걸로 대체할 수
> 있다.

| # | 절차 | 기대 결과 |
| :--- | :--- | :--- |
| M1 | Seek 세션을 열고(리매핑 키 또는 단축키) 한/영 키를 아무 modifier 없이 누른다 | **입력 소스가 전환되고 세션은 유지된다.** 세션 종료 로그가 남지 않는다. `키 입력` 로그에 한/영 키가 검색어로 들어가지 않는다 |
| M2 | M1 직후 영문 타이핑 | 검색어에 **영문**이 입력된다. 한글 조합(자모 결합)은 **안 됨** — 그것이 기대 동작이다(D2, IME 는 범위 밖). `query=…` 로그에 좁혀지는 것을 확인 |
| M3 | 세션 중 `a`·`;`·`Enter`·`Esc` 등 기존 키 | 기존 동작 그대로 (회귀 없음) — `a` 는 검색어, `;` 는 순환(☑ 시), `Enter` 확정, `Esc` 취소 |
| M4 | 한/영 키로 전환 후 **다시** 한/영 키 | 입력 소스가 **되돌아간다**(양방향) |
| M5 | 한/영 키를 **약 1초간 홀드** (autorepeat KeyDown) | **전환 1회, 왕복 없음.** macOS 가 lang1 반복을 억제하는지 실기기 확인 `(미확정)` — autorepeat KeyDown 이 그대로 통과돼도 lang1 은 원래 반복이 없다면 아무 일도 없어야 한다(§8, 리뷰 반영) |
...
## 17. UI/UX 1차 현대화 (이슈 #79·#84) — 수동 검증 (UXR-01~11)

> **자동 검증**: `cargo test --workspace`(회귀 전체) + `frontend_wiring.rs` 의 UXR 신규
> 정적 회귀 3건(`settings_html_의_선택_상태는_highlight_tint를_쓴다`·
> `settings_html_의_seek_미설정_경고는_warn_클래스를_가진다`·
> `settings_html_에_탭_전환_페이드_배선이_있다`). 여기는 **라이트/다크 양쪽에서 눈으로
> 확인**해야 하는 수용 기준만 남긴다(요구사항 문서 §2.1·§2.2 각 UXR 의 "수용 기준").
>
> **전제**: 서명된 `.app`(§0)을 `open` 으로 실행. 라이트/다크 전환은 시스템
> `System Settings → Appearance` (또는 메뉴바 제어 센터) 에서 변경한다. 설정 창
> 6탭·온보딩·Event Viewer·오버레이는 **각각 별도 확인**.

| UXR | 확인할 것 | 라이트 | 다크 | 통과 조건(눈으로) |
| :--- | :--- | :-: | :-: | :--- |
| UXR-01 | 사이드바 선택 탭·Keyboards 디바이스 목록 선택 행 | ☐ | ☐ | 선택된 항목이 **시스템 강조색 계열 채움**으로 보이고, hover(회색 8%)와 색 온도·강도로 즉시 구분된다 |
| UXR-02 | 설정 창을 Tab/⇧Tab 으로 순회 | ☐ | ☐ | 모든 포커스 가능 요소가 **동일한 형태(2px 강조색 링)** 로 포커스를 표시한다. 마우스 클릭에서는 링이 안 보인다. 체크박스·select·슬라이더는 네이티브 링 |
| UXR-03 | General 탭 accent 버튼(구매·라이선스 활성화)·충돌/복사 대화상자 계속 버튼 · 온보딩 `Open System Settings` | ☐ | ☐ | 라벨이 강조색 채움 위에서 대비되고, hover 시 한 단계 진해지며, disabled(구매)는 반투명 유지. `.danger`(비활성화 확인)는 붉은 스타일 유지 |
| UXR-04 | 섹션 헤딩(`h2.group`)·Event Viewer 컬럼 헤딩 | ☐ | ☐ | 대문자로 왜곡되지 않고(한글 포함), 라이트/다크에서 같은 대비 강도 |
| UXR-05 | 부제가 붙은 행 아래 간격·섹션 구분선(`hr`) | ☐ | ☐ | 힌트 있는 항목 아래가 힌트 없는 항목보다 넓고, 구분선 위아래 여유가 있다. 항목 순서·개수 불변 |
| UXR-06 | Seek 탭(출고 기본 상태) | ☐ | ☐ | "Seek has no way to open…" 이 **오렌지 경고**로 다른 부제와 구분된다. 활성 경로 하나라도 설정하면 사라진다. ⚠️ `.hint` 가 상속하는 `opacity: .65` 와 `.warn` 의 margin(4px 0) 이 겹치므로, **대비 강도·줄 간격을 눈으로 확인**한다(만약 너무 흐리면 경고 강도를 손봐야 한다 — 이 항목의 수용 기준) |
| UXR-07 | 탭 전환(6탭 전부 왕복) | ☐ | ☐ | 전환 시 패널이 **약 100ms 페이드**로 나타난다. "동작 줄이기" 켠 상태에선 즉시. 전환 후 컨트롤 렌더는 이전과 동일 |
| UXR-08 | Seek 탭 단축키 레코더 | ☐ | ☐ | 레코더가 일반 버튼과 구분되는 **필드형 캡슐**. 클릭 시 "Recording…" 상태에서 캡슐이 강조되고, 키 입력·Esc·Clear 동작은 기존과 동일 |
| UXR-09 | 온보딩(권한 모달 1차 화면)·out-of-sync | ☐ | ☐ | `Open System Settings` 가 설정 창 accent 와 같은 어휘. 3단계 수동 절차가 읽기 좋은 줄 간격. 힌트(자물쇠 안내) 대비가 라이트/다크 동일 |
| UXR-10 | Event Viewer — 키 몇 개를 Consume 시킨다 | ☐ | ☐ | 소비된(Consume) 행이 다른 행과 배경색으로 즉시 구분되고(강조색 12% + 좌측 막대), 위를 지나가도 consumed 로 보인다 |
| UXR-11 | Seek 를 라이트/다크 각각 발동(오버레이 표시) | ☐ | ☐ | 검색 바·하이라이트·선택·연결선이 `palette.rs` 출고값(라이트: 바 흰색 반투명·글자 `#1A1A1A`, 다크: 바 어두움·글자 `#F5F5F5`) 그대로 보인다. 세션 중 외관을 바꿔도 현재 세션 색은 변하지 않고 다음 세션부터 새 팔레트 |
...
> 결정 요지: 계획 issue-79 §7 수용 기준 — "사용자가 스크린샷 6장(6탭 라이트 각 1)을
...
| **M14-a** | (이슈 #102) 검색창 즉시 표시·입력 | `검색 언어` = `English + 한국어` 상태에서 캡스락 hold(또는 단축키)로 세션을 열고 **곧바로** 한글 쿼리 입력("설정") | 검색 바가 **즉시** 나타나고 `<input>` 에 포커스가 간다(쿼리 입력 가능). 카운터가 **"찾는 중…"** 을 표시한다 — "없음"이 아니다 | `docs/plan/issue-102-seek-ocr-ux.md` §3 D1 · F-03 §3.7 |
| **M14-b** | (이슈 #102) 검출 완료 전 잠정 매치 | 2개 디스플레이 환경에서 검출 종료(전체 ≈3.1s) **전에** 먼저 도착한 디스플레이 결과가 뜨는지 관찰 | 매치가 목록·하이라이트에 나타나도 **카운터는 검출 완료 전까지 "찾는 중…"을 유지**한다(잠정 매치 — 정상, 결함이 아님). 이 상태에서 Enter 가 도착한 첫 매치를 확정하는지(의도된 동작)와 ↑↓/Tab 순환이 무시되는지 함께 기록 | `docs/plan/issue-102` §2 사실 ⑨ · F-01 §5 #17 · F-03 §3.7 |
| **M14-c** | (이슈 #102) 검출 완료 후 확정 표시 | 검출 종료 후 카운터 확인 | "k / n" 으로 전환되고 매치가 그대로 필터링된다. (대조) 검출 완료 후 매치 0개 쿼리 입력 → "없음"(확정) | `docs/plan/issue-102` §3 D1 |
...
## 항목 22 — F-19 언어별 프리셋 (2026-09-03)

> ⭐ **근거**: `docs/spec/language-presets.md` §8 수용 기준 · §9 #1~#4 (`(미확정)` 유지) ·
> `../spec/korean-input.md` §3.2(0x66/0x68 키코드). 자동 테스트는 전부 macOS 없이 통과한다
> (`cargo test --workspace`) — 여기 있는 것은 **실기기·실제 키보드(JIS 등)·실제 입력 소스** 가
> 필요한 항목만 다룬다.

### 사전 준비

```sh
./scripts/build-signed.sh && open <경로>/Ultrakey.app
tail -f ~/Library/Logs/Ultrakey/ultrakey.log
```

`언어` 트리에서 확인할 프리셋을 켠다(아래 각 절의 조작 열). `settings.json` 위치는 기존
M1 절차와 같다.
...
| # | 시나리오 | 조작 | 기대 결과 | 판정 근거 |
| :--- | :--- | :--- | :--- | :--- |
| **M0** | 트리 | 환경설정 창 열기 | 사이드바에 `언어` 노드가 펼쳐져 있고 자식 `한국어`·`日本語`·`中文` 이 보인다. 다른 노드(`Presets` 등)와 크기·간격이 어긋나지 않는다 | 명세 §4.1 |
| **M1** | 트리 접힘 | `언어` 노드 클릭 | 자식 3종이 접히고 `aria-expanded` 전환. 창을 열었다 다시 열어도(재시작 후) **자식이 다시 펼쳐진다**(부모 펼침은 비영속 — 세션이 기억하는 것은 `ui.lastTab` 자식뿐) | §4.1 "기억" |
| **M2** | 탭 기억 | `日本語` 클릭 후 앱 종료 → 재시작 → 설정 창 | `日本語` 탭이 선택된 채 열린다. `settings.json` 의 `ui.lastTab` == `japanese` | F-15 §3.1 |
| **M3** | F-19.1 | `언어 ▸ 한국어` 에서 `캡스락 탭=한/영` 켜고 텍스트 편집기에서 캡스락 탭 | **1초 내 탭** → 입력 소스 전환. **길게** → 캡스락 잠금(대문자) | §8 |
| **M4** | F-19.2 | `오른쪽 command=한/영` 켜고 우⌘ 탭 / 우⌘+W | 탭 → 입력 소스 전환. **⌘+W 는 창 닫힘**(홀드로 확정 — modifier 유지) | §3.6 |
| **M5** | F-19.3 | `日本語 ▸ 캡스락=英数/かな` 켜고 일본어 IME → 캡스락 탭 / ABC → 캡스락 탭 | 일본어 소스에서 英数(영문), 비일본어에서 かな(히라가나) 로 전환. **JIS 키보드 필요 `(미확정)`** | §3.2 F-19.3 · 연구 §4-2 |
| **M6** | F-19.4 | `⌘ 단독 탭=英数/かな` 켜고 좌⌘ 탭 / 우⌘ 탭 / ⌘+W | 좌→英数, 우→かな. ⌘+W 는 홀드(창 닫힘) | §3.2 F-19.4 (안 A) |
| **M7** | F-19.5 | `¥↔\` 켜고 JIS 키보드에서 ¥ 키 탭 / `⌥`+¥ / US 키보드에서 `\`·`⌥\` | JIS: ¥↔`⌥¥`(=`\`). US: `\`↔`⌥\`(=¥). ⛔ `]` 가 나오면 안 된다 | §5 #10 (option 경유) |
| **M8** | F-19.6 | `JIS 키보드를 US 배열처럼` 켜고 JIS 키보드에서 §3.2.1 표의 키(shift+2·shift+6·¥·`\` 등) 입력 | US 배열 문자(`@`·`^`·`` ` ``·`\` 등) 가 입력된다. **JIS 키보드 필요 `(미확정)`** | §3.2.1 표 |
| **M9** | F-19.7 | `中文 ▸ 캡스락=중/영` 켜고 **제3자 입력기**(搜狗 등)에서 캡스락 탭 | 소스 전환. Apple 중국어 입력기에선 네이티브 전환이 있어 중복 감지가 안 될 수 있음 `(미확정)` | 연구 §4-3 · §9 #2 |
| **M10** | 게이트 | 원격 데스크톱 클라이언트(VNC/RDP) 를 최전면으로 두고 M3(캡스락 탭) 반복 | F-19 전환류가 **통과**(원격 시스템에 원래 키 전달). 같은 상태에서 hyper 는 **계속 동작** | §3.5 · §8 |

### ⚠️ 이 절차로 확인할 수 없는 것

- **macOS 한국어 입력기의 "캡스락으로 입력 소스 전환" 네이티브 옵션 존재 여부** (§9 #1,
  `(미확정)`) — 중국어 입력기는 Apple 문서로 확인됐으나 한국어는 실기기 미확인. 확인 방법:
  시스템 설정 ▸ 키보드 ▸ 입력 소스 ▸ 한국어 편집 화면.
- **Apple 중국어 입력기의 캡스락 네이티브 토글 실동작**(§9 #2, `(미확정)`).
- **3 개 이상 입력 소스에서 `⌃Space` 가 "특정 언어 직접 전환"** 인지(§9 #3, `(미확정)` —
  `⌃Space` 는 "이전 소스" 토글이라 순환 의미 추정). 3 소스 환경 실측 필요.
