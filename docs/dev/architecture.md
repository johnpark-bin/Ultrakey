...
```
 ...
 워치독 스레드       │ 1초 주기 CGEventTapIsEnabled 폴링        │
                    └──────────────────────────────────────────┘
                    ┌──────────────────────────────────────────┐
 지연 스케줄러       │ ("ultrakey-delay-scheduler")             │
 스레드              │  · 절전/세션/핫플러그 지연 예약          │
                    │  · ⭐ 경로 B 재적용(`apply_*`) 실행 주체  │
                    │    — hidutil·원장 fsync 는 여기서만       │
                    │    (이슈 #139)                           │
                    └──────────────────────────────────────────┘
```
...
| **탭 스레드로 보내는 명령**(상태 리셋, 탭 복구, 종료) | 메인·워치독 → 탭 | 무잠금 MPSC 큐 + **`CFRunLoopSource` (version 0)** 시그널 | 탭 스레드는 런루프 안에서 잠들어 있다. 큐에 넣고 `CFRunLoopSourceSignal` + `CFRunLoopWakeUp` 하면 런루프가 깨어나 perform 콜백에서 큐를 비운다 — **콜백과 같은 스레드에서 직렬 실행되므로 상태 테이블에 락 없이 접근할 수 있다** |

> ⭐ **커맨드 드레인·타이머 콜백도 탭 런루프 시간이다.** perform/타이머 콜백은 이벤트 콜백과 같은
> 스레드에서 직렬 실행되므로, 거기서 블로킹 작업(서브프로세스·fsync·네트워크·IOKit 열거)을 하면
> 그 시간만큼 시스템 전체 입력이 멈춘다(이슈 #139 실측: 재적용 179 ms = 입력 정지 179 ms).
> 그런 작업은 지연 스케줄러 스레드(또는 메인)에서 하고, 결과만 원자값으로 게시한다.
> 탭 스레드는 `QOS_CLASS_USER_INTERACTIVE` 로 올린다(`ultrakey-platform::thread_qos`) —
> WindowServer 가 활성 탭의 응답을 동기로 기다리므로. **이 결정 이후 경로 B 재적용은 탭 스레드로
> 보내는 명령이 아니다** — 지연 스케줄러 스레드가 직접 실행한다(§2.1 도식).

> ⛔ **콜백 안에서 절대 하지 않는 것** (컴파일러가 막아주지 못하므로 리뷰 체크리스트로 남긴다,
> 커맨드 perform·타이머 콜백 포함)
