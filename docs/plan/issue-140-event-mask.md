...
## 5. 실측 결과 요약 (2026-09-06)

상세 표·조건은 `docs/dev/input-latency-spike.md` §8.

| 판정 | 결과 | 근거 |
| :--- | :--- | :--- |
| A1 기본 구성 마스크 = 키 3종 | **확인** | 단위 테스트(core 9건·platform 3건 + macOS `CGEventType` 상수 대조) · 실기기 기동 로그 `mask=7168`(0x1C00) |
| A7 ② 유휴 → ① 수렴 | **확인** | ② p50 0.28 / 0.30 ms(2회) vs 같은 세션 ① p50 0.27 ms · #139 기준선 0.83 ms. 스파이크 0 |
| A5 FSM 경계 | **확인(테스트)** | `reconfigure_tap_decision` 5건, `handle_recover_tap` 본문 무변경(diff 대조) |
| A3 hyper + click 마스크 | **확인(테스트)** / 실기기 미측정 | `mouse_event_needs` 테스트(hyper 규칙·`hold_remap` modifier 프리셋) · 수동 확인은 사용자 미수행 |
