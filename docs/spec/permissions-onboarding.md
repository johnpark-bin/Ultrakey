...
#### ⭐ 3.2.1 신규 요구 — 모달은 **실패해도 눈에 보여야 한다** (이슈 #8 에서 추가)

이 절은 실제 사고로 추가됐다. 이슈 #8 에서 온보딩 모달이 "안 뜬다"고 보고됐으나, 실측 결과
**창은 정상적으로 최상단·포커스 상태로 떠 있었고 내용만 완전히 백지**였다. 문구를 가져오는
프런트엔드 코드가 첫 줄에서 예외를 던졌는데, 그 실패가 화면에도 로그에도 전혀 드러나지
않았기 때문이다. 사용자에게는 "모달이 안 뜬다"와 구별되지 않았고, 개발자에게는 원인을
가리키는 신호가 아무것도 없었다.

기존 §3.2 는 **모달이 무엇을 보여줘야 하는지**만 정의하고, **문구를 가져오지 못했을 때
무엇을 보여줘야 하는지**를 정의하지 않았다. 그 빈칸이 이 사고를 가능하게 했다. 따라서:

1. ⭐ **모달은 어떤 실패 상황에서도 백지로 남지 않는다.** 문구 조회(`modal_copy`)가
   실패하거나, 프런트엔드 초기화 자체가 실패하면, **실패했다는 사실과 그 사유를 창에
   표시**한다. "아무것도 안 보이는 창"은 허용되지 않는 상태다.
2. 그 진단 문구는 **문자열 카탈로그에 의존하지 않는다.** 카탈로그를 가져오는 경로가 죽었을 때
   쓰는 최후 수단이므로, 카탈로그에 기대면 같은 실패에 함께 휩쓸린다 — 이 한 곳에 한해
   D4(`localization-and-input-sources.md` §3.1.5 "문자열은 전부 카탈로그에서")의 예외를
   인정한다. **제품 문구가 아니라 오류 경로이기 때문이다.**
3. 창 배경은 **불투명**이어야 한다. 창 설정에 `transparent: true` 가 없는 한 투명 배경은
   얻을 게 없고, 렌더가 실패했을 때 "창이 텅 비어 보이는" 실패 모드를 더 나쁘게 만든다.
4. ⭐ **모달이 살아 있다는 것을 로그로 확인할 수 있어야 한다.** 프런트엔드가 실제로
   Rust 를 호출했다는 신호(`modal_copy` 호출 로그)가 남아야, "창이 안 떴다"와 "창은 떴는데
   내용이 없다"를 로그만으로 구분할 수 있다. 이슈 #8 에서는 이 구분이 불가능해 진단이
   크게 지연됐다.

⚠️ 1~3 은 `apps/ultrakey-app/tests/frontend_wiring.rs` 가 정적으로 검사한다(3번과, 1번의
가장 흔한 원인인 `withGlobalTauri` 누락). **실제로 화면에 무엇이 그려지는지는 자동
검증되지 않는다** — 그 부분은 `manual-verification.md` 항목 1-a #1 의 수동 확인이 소유한다.
...
10. **권한 모달이 다른 창 뒤에 뜬다.** 앱이 Dock 아이콘 없는 Accessory 앱(F-10)으로 동작할 때, 자체 모달이라도 다른 앱의 전체화면 창 뒤에 가려질 수 있다. `⌘Tab` 으로는 애초에 앱 자체가 노출되지 않으므로(F-10), 메뉴바 아이콘 클릭으로 모달을 다시 불러올 수 있는 경로가 항상 있어야 한다.
    ...
    ⭐ **실측(2026-08-30, 이슈 #8)**: `ActivationPolicy::Accessory` 상태에서 `WebviewWindow::show()` + `set_focus()` 만으로도 **모달은 최상단으로 나온다.** 로그 `is_visible=Ok(true) is_focused=Ok(true) is_minimized=Ok(false)` 와 스크린샷으로 확인했다. tao 의 `set_focus()` 가 `makeKeyAndOrderFront:` 에 더해 `activateIgnoringOtherApps:` 까지 부르기 때문이다(`tao` `platform_impl/macos/util/async.rs`). 따라서 **`NSApp.activate` 상당의 추가 처리도, `set_always_on_top` 도 M1 에는 필요하지 않다** — 이슈 #8 에서 이 가설(H1)을 세웠으나 실측으로 기각했다.

    다만 위 실측은 **전체화면 앱이 앞에 있는 경우를 포함하지 않는다.** 그 경우까지 커버하려면 별도 조치가 필요할 수 있고, 어느 쪽이든 "메뉴바에서 모달을 다시 부른다"는 경로는 F-10(M2) 이 소유한다 — 이 항목의 본래 요구는 유효하다.

11. ⭐ **모달의 웹뷰가 비어 있다(이슈 #8 실제 사고).** 창은 정상적으로 떠 있는데 내용이 렌더되지 않으면, 사용자에게는 "모달이 안 뜬다"와 구별되지 않는다. 원인은 `tauri.conf.json` 의 `app.withGlobalTauri` 누락(기본값 `false`)이었고, 그 결과 `window.__TAURI__` 가 주입되지 않아 프런트엔드 모듈이 첫 줄에서 `TypeError` 로 죽었다. §3.2.1 이 이 실패 모드에 대한 요구(진단 문구 표시·불투명 배경·`modal_copy` 호출 로그)를 정의한다.

12. ⭐ **권한 전이 콜백이 메인 스레드를 블록해 교착할 수 있다.** 폴링 스레드가 권한 모델 락을 쥔 채 전이 콜백을 부르고, 그 콜백이 `show_modal()` → macOS 메인 스레드로 **동기 디스패치**되는 반면, 메인 스레드는 `modal_copy` 커맨드에서 같은 락을 기다리는 조합이다. 이슈 #8 이전에는 `modal_copy` 가 한 번도 호출되지 않아 도달 불가능했으나, 모달이 실제로 뜨는 순간부터 도달 가능해진다. **전이 콜백은 반드시 락을 놓은 뒤에 호출한다**(`ultrakey-permissions` `monitor.rs`).
