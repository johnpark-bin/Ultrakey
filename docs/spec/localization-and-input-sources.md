...
⭐ **이슈 #131 (2026-09-05, D11)** — Seek 검색 언어(`seek.searchLanguage`)는 `preferences-ui.md` Seek 탭 소관이며 **부재 = 영어 고정**(`Locale::En` → 빈 목록 = Vision 기본). 이슈 #48 의 '부재=UI 로케일 폴백'은 이로써 폐기 — Seek OCR 언어가 UI 로케일에서 파생되는 경로는 없다. 근거: 영어 기본 = Vision 기본(성능·신뢰도), 이슈 #93 선택 UI 존재, 사용자 피드백('선택했을 때만'). 상세: seek-text-detection.md §3.2.2
