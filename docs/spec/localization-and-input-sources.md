...
⚠️ **`settings.tab.*` 라벨의 번역 범위가 로케일마다 다르다.** `ko` 는 `Presets`·`General`·`Korean` 을 영어로 두고 `Keyboards` 만 번역했으나, `zh`·`es`·`ja` 는 브랜드가 아닌 일반 명사를 전부 번역했다. 후자가 §3.1.3 의 분류표(브랜드·기능 고유명사만 비번역)에 맞고, `ko` 쪽이 그 표와 어긋나 있다 — **정정 대상은 `ko` 이지만 이번 회차에서 고치지 않았다**(같은 파일을 병렬 작업 #40 이 건드리고 있어 충돌을 만들지 않기로 했다). §9 미해결 질문에 남긴다.
