...
⭐ **16개 기능 중 "네이티브 shim 불가피" 는 없다.** v1.66 실측이 이 판정을 뒤집지 않았다 — 원본이 쓰는 프레임워크는 전부 C ABI 또는 Objective-C 로 노출된다. ⭐ 나중에 추가된 F-16(한국어 입력 지원)도 이 판정을 바꾸지 않는다 — 새로 도입하는 플랫폼 API 가 하나도 없고, F-07·F-14 가 이미 쓰는 FFI 를 그대로 재사용한다. 근거와 그럼에도 남는 위험은 [`platform-constraints.md` §0·§6](platform-constraints.md) 에 있다.
...
⭐ **F-16 은 두 단계로 나눠 구현한다** — 절반이 `(미확정)` 위에 서 있기 때문이다.

| 단계 | 범위 | 착수 조건 |
| :--- | :--- | :--- |
| **1단계** | `Shift + Space` → 입력 소스 변경 · `₩` → `` ` ``(한국어 입력기일 때만) · 기능별 앱 제외 게이트 · `Korean` 탭 UI(ko + en 문자열) | **지금 가능하다.** 필요한 사실이 전부 확정되어 있다 |
| **2단계** | 한/영(`lang1`) → 입력 소스 변경 · 한자(`lang2`) → 한자 변환 | ⛔ **`korean-input.md` §3.2 의 keycode 실측이 선행되어야 한다.** 그전까지 UI 항목은 dimmed 로 출하한다 |
