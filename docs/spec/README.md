...
**정리** — 표는 아홉 행이다. **새 탭을 만드는 갈라짐은 F-16 에 이어 F-17 이 두 번째**이고, D4(현지화)와 그 확장인 D6 는 기존 UI 위에 얹히는 횡단 선택이라 성격이 다르다. **D5·D9 는 성격이 또 다르다** — 아래 기준 1을 통과하지 못하는 **이탈**이며, 둘 다 "사용자가 원본의 동작을 실제로 써 보고 명시적으로 다른 것을 요구"한 것이 허용 근거다(D9 는 이슈 #77: "접근성이 매우 나쁨" — 트레이 메뉴 전용 배치를 사용자가 직접 실경험한 뒤 이동을 요구).
