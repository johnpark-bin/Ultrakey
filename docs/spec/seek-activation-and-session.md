...
- ⭐⭐ **인풋 박스 모드 세션 중 입력 소스 전환 3경로 (이슈 #101 — #76 의 확장)**: 다국어 세션에서도 입력 소스 전환은 살아 있어야 한다 — "전환된 상태에서의 입력은 정상"이라는 사용자 관찰(#101)이 전제하는 그대로다. 세 경로를 아래처럼 다룬다.
  ...
  - ⭐⭐⭐ **이슈 #125 실측·수정 — #121 후보 E-②(hyper 슬롯 잔류)를 코드로 확정하고 D2 판정을 수정했다 (결정 기록)**: #121 이 `(미확정)`으로 남긴 탭 스레드 런타임 후보 중, hyper 소스 키(예: RIGHT_COMMAND)가 세션이 열리기 **전**에 `HoldConfirmed` 로 확정된 뒤 세션 중 물리적으로 뗀 경우가 D2 를 막는다는 것을 코드로 확인했다: `is_tracked`(FSM 진입)는 세션 게이트 **뒤**라(#121 결정 그대로) 그 릴리즈가 FSM 에 도달하지 못해 슬롯이 `HoldConfirmed` 로 잔류하고, D2 가드(`active_synth_flags` 가 빈 것)가 이 stale 잔류를 "진짜 hyper 활성"으로 오판해 F-16.1 을 계속 막는다. **수정**: D2 가드를 `active_synth_flags_of_pressed_slots()`(정본 눌림 테이블로 stale 을 걸러낸 값)로 교체 — 물리적으로 눌려 있는 진짜 hyper hold 는 여전히 막고, 물리적으로 뗀 stale 잔류만 통과시킨다. 판정 로직은 D2 가드 한 곳만 바뀌었고, ①②③ 경로 자체·세션 밖 F-16.1·다른 `active_synth_flags` 소비처는 무변경. "정확히 3번째" 주기 수치와 stale 이 애초에 왜 생기는지(정상 해제 유실의 실기기 트리거)는 여전히 `(미확정)` — 이번 수정이 닫는 것은 hyper 합성 flags 잔류가 D2 를 막는 경로 하나다. 절차는 `docs/plan/issue-125-seek-shiftspace-hyper-stale.md` §8-6 M10(확장 트레이스 필드 `synth_flags_active_pressed` 포함).
  - ⭐⭐⭐ **이슈 #127 실측·수정 — ③(⇧+Space)의 Space 자동 반복(autorepeat)이 D2 합성을 반복마다 재발화했다 (결정 기록)**: #125 포함 빌드에서도 재발한 별개 경로 — #121/#125 는 "래치·게이트가 잘못 발화를 막는" 방향의 결함이었던 반면, #127 은 그 반대로 **발화가 너무 많이 일어나는** 결함이다. 사용자 macOS 키 반복 설정(`InitialKeyRepeat` 225ms 근방)에서 ⇧+Space 를 그 이상 누르고 있으면 물리 SPACE **autorepeat** KeyDown 이 반복 도착하는데, D2 가 재사용하는 `evaluate_korean_rules` 는 원래 `ev.autorepeat` 를 보지 않아 반복 down 마다 합성 `⌃Space` KeyDown 을 매번 새로 방출했다(래치는 덮어쓰기만) — 합성 이벤트에 autorepeat 플래그가 없어 macOS 가 매 반복을 독립된 토글로 오인해 **입력 소스가 반복마다 되토글**됐다(실기기 로그 — 208ms 뒤 Korean→ABC). **수정**: F-16 규칙에 `is_input_source_toggle` 플래그를 추가해 F-16.1·F-16.2(이 경로가 다루는 SPACE 치환) 만 "래치가 이미 서 있으면 autorepeat KeyDown 을 재발화 없이 소비"하도록 좁혔다 — 판정 로직은 `evaluate_korean_rules` 내부 한 곳(규칙 매칭 직후, 게이트 검사 앞)이고, D2·계층 3(세션 밖) 양쪽에 동일하게 적용된다(같은 함수 공유). 실측·설계·대안 비교는 `docs/plan/issue-127-shiftspace-autorepeat.md`, F-16.1 행 결정 기록은 `korean-input.md` §3.1.
  - ⭐⭐⭐ **이슈 #129 실기기 트레이스·수정 — ③(⇧+Space)의 합성 `⌃Space` 가 웹뷰에 흡수돼 6번 방출에 1번만 전환됐다 (결정 기록)**: `ULTRAKEY_TRACE_TAP=1` 실기기 트레이스(검색 언어 "영어+한국어", 인풋 박스 세션 중)가 연속 ⇧+Space 6회 **전부** `layer=KoreanInput disp=Consume emitted=[KeyDown 0x31 0x40001]` 로 D2 가 정상 발화·방출함을 보였지만, `kTISNotifySelectedKeyboardInputSourceChanged`(실제 입력 소스 전환)는 **1회뿐**이었다 — 방출:전환 = 6:1(세션 밖은 1:1). 저장소는 **같은 웹뷰-키-윈도우 상태에서 물리 `⌃Space`(②)가 이미 전환된다고 명세돼 있으므로**(§3.1 ②, 위) 흡수는 "웹뷰가 키 윈도우"라서가 아니라 **합성 이벤트 고유**의 현상으로 판정됐다. **검토한 대안과 기각**: (a) D2 발화 시 `TISSelectInputSource` 로 직접 전환 — **기각**. macOS 는 CJK(중·일·한·베트남) 입력 소스로의 `select_input_source`/`TISSelectInputSource` 직접 전환에 문서화된 버그가 있다(Karabiner-Elements 공식 문서 + 독립 보고 3건 — "아이콘은 바뀌는데 실제 입력은 안 바뀐다"). F-16.1 이 애초에 `⌃Space` 재주입을 쓰는 이유가 이 버그이고(`language-presets.md` §8), 이번 전환 목표(한국어 IME)가 그 버그의 트리거 조건과 정확히 겹친다 — 채택 시 실패 양상이 지금(안 눌림)보다 조용히 더 나쁠 수 있고 CI 로 방어할 수 없다. **`language-presets.md` §8 의 부정형 수용 기준은 이번 이슈에서 무수정이다.** (b) 관측 후 유계 재시도(전환 미확인 시 합성 ⌃Space 재발출) — **기각**. 전환 지연이 가변적이라 재시도 창이 그 지연보다 짧으면 되토글(#127 이 실측한 것과 같은 조용한 실패 양상)이 난다. **채택한 수정**: D2 의 판정 로직(`evaluate_korean_rules`)은 한 줄도 바꾸지 않고, **인풋 박스 세션 중 D2 가 낸 합성 이벤트의 전송 경로만** 바꾼다 — 탭 콜백 안(`post_to_tap` = `CGEventTapPostEvent`, 물리 SPACE 이벤트를 처리 중인 바로 그 콜백 스택 프레임)이 아니라, 기존 커맨드 큐(`RecoverTap` 이 쓰는 것과 같은 경로)를 거쳐 콜백 밖(`post()` = `CGEventPost`)에서 낸다(`ultrakey-engine::engine::emit_outcome`/`should_defer_synth_to_command_queue`, `EngineCommand::PostSynthEvent`). 게이트는 `gates.seek_active && gates.seek_input_box && outcome.layer() == Layer::KoreanInput` — 세션 밖 같은 계층 산출(위 F-16.1 세션 밖 발화)은 `seek_active == false` 라 이 분기를 타지 않는다. **완료 판정은 실기기 한정**(`(미확정)`) — `ultrakey-core` 의 F-16.1·인풋 박스 테스트는 판정 로직이 무변경이라 그대로 통과하지만, "방출이 실제로 어떻게 전달되는가"는 이 크레이트만으로 고정할 수 없다. 실측·후보 비교·수용 기준·M-절차는 `docs/plan/issue-129-seek-webview-inputsource.md`(§7 이 P2 확정).
