...
⭐⭐ **해소 (M3 2차, 이슈 #34 — 전문 `docs/dev/seek-overlay-render-spike.md`)**

실측했고, **교체 조건이 발생하지 않았다.** 같은 기기(M4 Pro, 3840×1600 + 2560×1440)에서 `Rust emit → WKWebView 렌더 → paint → ack` 왕복:

| 후보 수 | 절대배치 DOM | **`<canvas>` 2D** |
| ---: | ---: | ---: |
| 0 (바닥값) | 15.4 ms | 25.0 ms |
| 200 | 42.3 ms | ✅ **24.7 ms** |
| 500 | ⚠️ 92.5 ms | ✅ **24.8 ms** |
| 1000 | ⛔ 191.4 ms | ✅ **24.8 ms** |

- ⭐ **답은 "WKWebView 가 느리다"가 아니라 "DOM 이 느리다" 였다.** Canvas 는 후보 수에 **전혀 반응하지 않는다**(0개 25.0 ms, 1000개 24.8 ms — 바닥값은 60 Hz 디스플레이의 더블 rAF 대기다). JS 실행 시간은 양쪽 다 1~5 ms 이고, DOM 의 나머지 176 ms 는 전부 **수백 개 요소의 레이아웃·합성**이다. Rust 쪽 직렬화는 1000개에서도 0.3 ms 다.
- **확정**: WKWebView + `<canvas>` 2D. `CALayer` 교체는 하지 않는다.
- **그럼에도 트레이트 분리는 유지한다**(`ultrakey_overlay::renderer::OverlayRenderer`). Retina(배율 2.0)·창 3개 이상·저사양 기기는 아직 재지 않았고, 재측정 결과가 나쁘면 그 자리에 `CALayer` 구현이 들어온다. **이 이음매가 있어서 위 결정을 안전하게 되돌릴 수 있다.**
...
| P3 | WKWebView 오버레이 렌더링 지연 | ⭐⭐ **전량 해소(이슈 #34).** 나머지 절반을 실측했다 — `<canvas>` 2D 로 그리면 후보가 **0개든 1000개든 왕복 중앙값이 약 25 ms 로 같다**(p95 28.3 ms). 즉 후보를 그리는 추가 비용이 사실상 0 이고 §3.6 예산 안이다. 반면 절대배치 **DOM 은 500개 92.5 ms(p95 108.6) · 1000개 191.4 ms** 로 예산을 넘는다. ⭐ 즉 위험은 IPC 왕복이 아니라 **DOM 레이아웃·합성**이었다(JS 실행 시간은 양쪽 다 1~2 ms). **WKWebView + Canvas 로 확정하고 `CALayer` 교체는 하지 않는다**(단, 트레이트 이음매는 유지 — Retina·저사양은 미측정). 근거: `docs/dev/seek-overlay-render-spike.md`. ─── 이하 이슈 #30 이 닫은 절반: **검출 쪽도 실측됐다** — **검출 쪽은 실측됐다** — 전체 화면(3840×1600 + 2560×1440) `.accurate` OCR 이 **약 775 ms**(캡처 31 ms + OCR 744 ms)이고, `VNRecognizeTextRequest` 는 **병렬화되지 않는다**(이득 0 %). ⭐ 중요한 것은 이 비용이 **세션당 1회**이고 매 키 입력마다 드는 비용이 아니라는 점이다 — 입력마다 하는 일은 이미 만들어진 후보 목록에 대한 순수 문자열 필터링이다. **따라서 P3 가 남긴 진짜 미실측은 "이미 그려진 오버레이를 키 입력마다 다시 그리는 WKWebView 왕복" 하나로 좁혀졌다.** 근거: `docs/dev/seek-ocr-latency-spike.md` | ⭐ 새로 파생된 미확인 3건: **Retina(`backingScaleFactor = 2.0`)** · 창 3개 이상 · 저사양 기기(스파이크 §5) |
