...
| `recognitionLanguages` | ⭐ **미설정(Vision 기본값 = 영어)을 기본으로 하고, 한국어는 사용자가 켜는 설정으로 노출** — **실측 확정**(승격) | ⭐⭐ **실측**(`docs/dev/seek-ocr-latency-spike.md` §3): ① Vision 의 기본 인식 언어는 **영어뿐**이다 — 화면이 한글로 가득해도 후보가 **0개**다(조용한 실패). ② `"ko-KR"` 은 목록의 **첫 번째**여야 동작한다: `["en-US", "ko-KR"]` 의 결과는 `["en-US"]` 와 관측 수·시간·신뢰도까지 **완전히 동일**했다. ③ 대가가 크다 — `["ko-KR", "en-US"]` 는 전체 화면 775 ms → **약 3.1 s**(3~4배), 평균 신뢰도 0.83 → 0.59. 그래서 기본으로 켜지 않는다(결정 **S-4**). 기존의 "8개 로케일 번들" 근거는 §5 #10 이 이미 오독으로 정정했다. |
