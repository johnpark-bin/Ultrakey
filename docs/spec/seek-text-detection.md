...
### 3.7 ⭐(이슈 #134 · D13) 세션 간 검출 결과 캐시 — 재진입 캐시 우선

⭐ 원본 SuperKey 에는 없는 **클론 고유 메커니즘**이다(갈라짐 표 D13). 사용자 명시 요구(이슈 #134 원문): "기존에 있던 데이터에서 우선 탐색을 하고 OCR를 돌리는 스레드가 OCR 결과를 만들어냈을 때 가장 최신화된 데이터를 가지고 다시 한번 탐색을 하거나" — **세션마다 신규 OCR 을 처음부터 기다리는 대기**(영어 약 0.8 s, 다국어 약 3.1 s — §5 #8)를, 직전 세션의 검출 결과를 **세션 밖에 보존**해 재진입 시 없앤다. ⚠️ **이 캐시는 소비자 계층(F-01 세션 머신·오버레이·컨트롤러)에서만 동작한다 — `detect.rs` 를 건드리지 않는다.** 검출 파이프라인(소스 A/B/C·M1~M4 병합)·`spawn_detection` 신호 3종·세대 판정은 전부 무변경이며, 이 절은 그 출력을 세션 사이에 이어 주는 얇은 층일 뿐이다(재진입 시퀀스: `seek-activation-and-session.md` §3.3). 설계 결정 5종 및 기각한 대안 3종(현행 유지·캡처 이미지 캐시·디스크 캐시)의 전체 근거는 이슈 #134 결정 코멘트(comment-5548276969)에 있다 — 여기서는 결정만 확정으로 반영한다.

**구조 (결정 1) — `SeekDetectionCache { candidates: Vec<TextCandidate> }`.** 마지막으로 **완료**된 검출의 후보 **전체** — OCR(소스 A)·AX(소스 B)·창 제목(소스 C) 세 소스를 병합한 결과로, 후보별 `text`·`frame`·`source`·`confidence`·`display_id`·`window_id`·`pid` 가 모두 담긴 `TextCandidate` 그대로 — 를 **세션 밖인 `SeekController`** 필드에 `Option<SeekDetectionCache>` 로 보관한다. 후보를 세션-스코프 객체(`OverlaySession`)에만 두고 세션 종료 시 드롭하는 현행 구조는 그대로 두고, 캐시만 그 별도 구조다. ⚠️ **메모리 한정, 디스크에 쓰지 않는다**(제품 제약 AGENTS.md §4 — 처리한 화면 데이터 비저장). 타입은 순수 로직 크레이트 `ultrakey-seek-session` 의 새 모듈 `cache.rs` 에 둔다 — macOS 비의존이라 순수 로직 테스트가 가능하다.

⭐ **재진입 주입 시의 디스플레이 필터**: 캐시 후보를 주입할 때, **새 세션의 디스플레이 목록에 없는 `display_id` 의 후보는 주입하지 않는다** — 세션 사이 디스플레이 구성이 바뀌어(외장 모니터 분리 등) 사라진 디스플레이의 후보가 유령 매치로 남는 것을 막는다. 이는 기존 `set_displays` 의 "사라진 디스플레이 후보 폐기" 규약과 같은 결이며, "신규 OCR 이 캐시 후보를 통째로 교체해 stale 이 유계다"는 만료 논증을 **세션 간 디스플레이 변경 케이스에서도** 보존하는 구멍 메꿈이다 — 사라진 디스플레이의 후보는 신규 OCR 을 기다릴 필요 없이 주입 시점에 이미 걸러지므로 잠시도 나타나지 않는다.

**갱신 (완료 결과만, 1회).** 캐시는 **현재 세대**의 `DetectionFinished` 에서, **세션이 아직 살아 있을 때** 오버레이 상태 스냅샷(`all_candidates` = 디스플레이별 `by_display` + `extra`)으로 **1회** 갱신된다. 그 시점의 오버레이 상태는 병합된 검출 결과와 같은 내용이다 — 검출 워커는 병합된 `DetectionOutcome` 을 직접 받지 않고 디스플레이별·extra 신호만 받으므로, 완료 시점의 오버레이 상태가 같은 병합 입력의 결과다. ⚠️ 세션이 먼저 닫혀 `overlay() == None` 이면 **갱신하지 않는다**(기존 값 유지) — 취소된 세션의 늦은 결과로 캐시를 더럽히지 않는다. **부분 결과는 캐시하지 않는다** — 디스플레이별 `Candidates` 도착마다 갱신하면 직전 세션의 *완전한* 결과를 조각으로 덮어써 재진입 첫 탐색의 질을 오히려 떨어뜨린다. 완전한 결과만, `DetectionFinished` 한 번에만 쓴다.

**만료 (결정 2) — 단일 슬롯, TTL 없음.** 캐시는 **단일 슬롯**이며 "직전 세션의 것만"을 문자 그대로 따른다 — 마지막에 완료된 검출이 그대로 다음 것이다. 시간 만료(N초)·화면 변화 감지가 없다. 기각한 대안:
- **시간 기반 TTL** — 마법의 상수 하나가 추가되고, 무엇보다 TTL 안의 캐시도 화면이 바뀌었으면 어차피 stale 이라 **만료가 stale 위험을 없애지 못한다**.
- **화면 변화 감지(주입 전 캡처 이미지 해시 비교)** — 주입 전에 캡처를 한 번 더 떠야 해 **"즉시 탐색" 목표와 정면으로 충돌**하고 캡처 비용(≈31 ms, §5 #8)이 늘어난다. 재진입 빈도 대비 이득이 없다.

stale 노출 창은 **유계(bounded)다** — 재진입 때마다 항상 도는 배경 신규 OCR 이 세션 열림 ~0.8–3.1s 뒤 후보를 통째로 교체한다(아래 최신화).

**세대 조합 (결정 3) — 폐기 유지, 캐시 갱신으로 라우팅하지 않는다.** 늦게 끝난 **이전 세대**의 `Candidates`/`ExtraCandidates`/`DetectionFinished` 는 현행 그대로 **조용히 폐기**하고, 캐시 갱신 경로를 추가하지 않는다. 근거: ① 캐시의 정상 갱신 경로는 **그 세션이 살아 있는 동안**의 `DetectionFinished` 다 — 세션 N-1 의 OCR 이 N-1 중에 끝나면 그 결과가 이미 캐시에 들어간다. 폐기 대신 라우팅을 해 얻는 이득은 "세션 N 열림 직후 N-1 의 OCR 이 늦게 끝난" **희귀 레이스**에서 캐시가 N-2 대신 N-1 결과를 갖는 것뿐이고, 그때 두 결과의 화면 상태는 사실상 같다(수백 ms 차이). ② 라우팅 구현은 **세대별 부분 상태 누적**(디스플레이별 + extra 버퍼, 수명·메모리 관리)을 요구해 복잡도가 실질 이득보다 크다. ③ "소비자 계층만" 경계(`detect.rs` 무변경)에 가장 부합한다. ⭐ 늦은 이전 세대 결과가 **현재 세션에 섞이지 않는다**는 보안 성질은 기존 `generation` 비교가 그대로 지킨다(변경 없음). **현재 세션의 후보 주입 경로(`ingest_display`/`ingest_extra`)는 한 줄도 바뀌지 않는다.**

**stale 표시 (결정 4) — 신뢰 위임(사용자 책임).** 캐시 기반 후보는 그대로 하이라이트되고 **클릭 가능**하며, 확정 시 재검증 캡처를 하지 않는다. 근거: ① stale 노출 창이 **유계**다(결정 2) — 신규 OCR 이 ~0.8–3.1s 후 후보를 통째로 교체하므로 stale 후보가 남는 시간이 짧다. ② **같은 실패 클래스가 현행에도 이미 존재한다** — §5 #9(세션이 열린 채 화면이 바뀌는 애니메이션·스크롤·알림)의 스냅샷 어긋남과 같은 종류다. 캐시는 스냅샷의 나이(세션 열림 시점 → 직전 세션)만 늘릴 뿐, 실패의 *종류*를 새로 만들지 않는다. 기각: **확정 전 재검증 캡처**(확정 핫 경로에 블로킹 캡처를 넣어 지연을 다시 만든다) · **stale 후보 표시 약화**("캐시에서 온 후보"라는 출처를 후보에 실어 새 주입 경로와 UI 변경을 요구 — 주입 경로 불변·"소비자 계층만" 경계에 어긋나고, 캐시 후보는 새 OCR 도착 즉시 교체되는 잠정 상태라 영속 UI 표현이 과하다). ⚠️ 남는 위험(스냅샷 좌표 vs 실시간 화면)은 수동 검증 절차에 "화면 변경 직후 재진입 시 클릭 좌표는 신규 OCR 완료 후 최신화된다" 항목으로 남긴다.

**최신화·선택 인덱스 (결정 5).** 재진입 후 기동한 `spawn_detection` 의 신규 OCR 결과는 **기존 `ingest_display` 의 "같은 `display_id` 교체" 의미론**으로 캐시 후보를 덮는다 — **재탐색은 별도 코드가 아니라 기존 증분 수신(이슈 #102)이 실현**한다. 캐시 후보와 신규 후보의 병합·중복 제거 규칙은 §3.4 그대로다. **선택 인덱스 유지 규칙**: ① **재진입(캐시 주입)은 인덱스 0 에서 시작**한다 — 새 세션이며 일반 세션 열림과 동일하다. ② **최신화(신규 OCR 이 캐시 후보를 교체) 중에는 인덱스를 유지**하되, 새 목록 범위 밖이면 마지막 유효 인덱스로 당긴다(**clamp** — `recompute_matching` 의 기존 동작 그대로며, `set_query` 처럼 0 으로 리셋하지 **않는다** — ingest 는 쿼리가 아니다). 사용자가 위치시켜 둔 선택이 목록 길이가 허용하는 한 유지된다. 기각: **텍스트 기반 선택 보존**(재계산 전 선택 텍스트를 캡처해 새 목록에서 같은 텍스트를 찾기) — `set_query` 의 "선택 0 리셋" 계약을 침범하거나 별도 경로가 필요하고, 인덱스 + clamp 의 예측 가능성·단순성 대비 이득이 작다. 이 규칙은 오버레이 순수 로직 테스트로 고정한다.
...
| 17 | ⭐ (신규, 이슈 #134) 세션 사이에 화면이 바뀜(창 이동·앱 전환) | 재진입 시 **남아 있는 디스플레이의** 캐시 후보가 잠시 그대로 보이고 클릭 가능하다(§3.7 신뢰 위임 — 결정 4) — 그 좌표는 직전 세션 기준이라 실시간 화면과 어긋날 수 있다. ⚠️ 단, 세션 사이 디스플레이 구성이 바뀌어 **사라진 디스플레이의 캐시 후보는 주입 시점에 이미 걸러진다**(§3.7 — 새 세션의 디스플레이 목록에 없는 `display_id` 는 미주입) — 유령 매치로는 잠시도 나타나지 않는다. stale 노출 창은 유계다: 재진입 때마다 도는 배경 신규 OCR 이 완료(~0.8–3.1s)되면 기존 `ingest_display` 교체 의미론으로 남아 있는 후보가 최신화되어 어긋남이 해소된다. 좌표 어긋남 가능성은 §3.7 의 신뢰 위임 정책으로 흡수한다 — 재검증 캡처·표시 약화 없음 |
