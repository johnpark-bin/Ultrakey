...
### 시나리오 D — 이중 실행 시도 (**⭐ 이슈 #92 실측 확정**)

사용자가 실행 중인 앱을 "다시 실행"하는 실제 경로에는 **두 번째 프로세스가 생기지 않는** macOS 동작이 있다. 클론은 두 경로를 나눠 처리한다:

1. **Finder 더블클릭 / `open`(사용자의 실제 재실행 경로)** — macOS LaunchServices 는 같은 번들 ID 의 프로세스가 이미 실행 중임을 감지하면 **두 번째 프로세스를 만들지 않는다.** 대신 `kAEReopenApplication`(reopen) Apple Event 를 실행 중인 인스턴스로 보낸다. ⭐ **실측 확정(이슈 #92)** — `LSUIElement` 액세서리 앱 검증 앱으로 `open` 재실행 시 두 번째 프로세스 없이 이 콜백이 도착하는 것을 확인했다. 클론은 이 이벤트를 받아(tao `applicationShouldHandleReopen:` → `tauri::RunEvent::Reopen`) **설정 창을 연다**(이슈 #92). 이미 떠 있으면 `show_settings_window` 의 `is_visible` 분기가 포커스만 준다 — 중복 창이 생기지 않는다(이슈 #68 체크리스트 2, F-09 §3.3).
2. **`open -n` 등으로 진짜 두 번째 프로세스가 뜬 경우** — 두 번째 프로세스는 시작 직후 같은 번들 ID 인스턴스가 있음을 감지하고(`other_instance_running`, 1-b) 기존 인스턴스에게 분산 알림 `app.ultrakey.Ultrakey.showSettings`(이슈 #68)을 보낸 뒤 **즉시 종료**한다. 기존 인스턴스는 알림을 받아 같은 `show_settings_window` 로 설정 창을 연다. 분산 알림 전달은 두 프로세스가 같은 번들 ID 를 공유해도 동작함을 확인했다(이슈 #92 재현 실측).
3. 어느 경로든 새로운 `CGEventTap` 은 설치되지 않는다 — 키 입력이 두 번 처리되지 않는다(§5 항목 1).

> ⛔ 구버전이 "Finder 재실행 = 두 번째 프로세스가 감지·즉시 종료"로 서술한 것은 실측으로 **틀렸다** — Finder 재실행은 프로세스가 아예 생기지 않고 reopen Apple Event 가 온다. 두 번째 프로세스가 즉시 종료되는 것은 `open -n` 같은 강제 경로에서만 관찰된다(7-e, 이슈 #92).
...
| `About` | F-10 (정보 행 내용은 F-09·F-13 과 공유) | **재현 → 독립 창 확정(이슈 #87).** 원본엔 `About.storyboardc` 존재로 About 창이 있음이 확인됐다(실측: 번들 구조). 클론도 **별도 정보(About) 창**(`ui/about.html`, `show_about_window`, 이슈 #87)으로 재현한다 — ⚠️ 옛 결정("별도 About 창을 만들지 않고 설정 창 General 탭에 두었다", `main.rs` 4987~4994 행 — 현재 약 5192 행, 커밋 `eac7b62`)은 사용자 피드백(2026-09-02)으로 **뒤집혔다**. About 창은 `plan/issue-87-about-window.md` D1~D6 을 따른다(정보 행 · `Check for Updates…` 버튼(F-13 재사용) · 닫아도 숨김 상주(#88 정책) · 카탈로그 교체 시 이벤트로 즉시 갱신). ⭐ **이슈 #96(2026-09-03) — UI 개선.** ① **앱 아이콘 노출** — 히어로 상단에 정본 SVG 의 인라인 사본(`docs/dev/icons.md` §2.4 관례 — 온보딩 모달과 같은 방식. `<img>`·asset 프로토콜 기각 근거는 `ui/about.html` 주석)을 `currentColor` 선화로 표시해 라이트/다크를 자동 대응한다. ② **레이아웃** — 히어로(아이콘·앱 이름·버전) + 정보 행 6종 + 하단 중앙 버튼. 앱 이름·버전을 히어로가 직접 표시하므로 라벨 행 2종과 카탈로그 키 `about.app_name`·`about.version` 은 제거했다. ③ **창 크기 — 스크롤바 없는 고정 크기**: 5개 언어 × 번들 내·외 2상태 전부를 헤드리스 렌더로 실측해 가장 높은 조합에 여백을 더한 **540×480pt** 로 고정하고 `resizable(false)` 유지. 본문은 세로 중앙 정렬(짧은 조합은 위아래 여백 균등). 기각: JS 콘텐츠 측정 후 리사이즈(로딩 깜빡임·상주 창 재측정 복잡도) — 내용은 언어와 번들 여부만 타는 고정 행 구성이라 정적 실측으로 충분하다. |
...
⭐ **재실행(reopen) 경로와의 관계 (이슈 #92 실측 확정)** — 원본이 `Hide menu bar icon` 복구 안내로 쓰는 "relaunch from Finder to open."(§4)이 가리키는 "Finder 재실행" 은 macOS 상 **애초에 두 번째 프로세스를 만들지 않는다.** LaunchServices 는 실행 중인 인스턴스로 `kAEReopenApplication` Apple Event 를 보낼 뿐이며, 앱이 그것을 처리하지 않으면 사용자에게 아무 일도 일어나지 않는 것처럼 보인다 — 그것이 이슈 #92 의 원인이다(실측: `LSUIElement` 검증 앱). 클론은 이 이벤트를 `tauri::RunEvent::Reopen` 으로 받아 설정 창을 연다(이슈 #92, §2 시나리오 D).
...
| `Visit superkey.app` / `About Superkey` / `AboutWindowController` | About 관련 문자열·컨트롤러 | §3.3 `About` 항목과 연결. ⭐ 이슈 #87 로 **클론의 독립 About 창**(`ui/about.html`, `show_about_window`)으로 재현 확정 — 이 문서의 옛 "최소 정보 창" 판단을 구체적 구현으로 깎은 것(AX 실측은 못 했지만 원본에 About 창이 있음은 `About.storyboardc` 가 보증, §3.3). |
...
1. **이중 실행.** 두 프로세스가 동시에 뜨면 각자 `CGEventTap` 을 설치하려 시도해 키 이벤트가 두 번 처리된다. §3.2 의 단일 인스턴스 보장으로 두 번째 프로세스가 즉시 종료돼야 한다(§6, §7). ⭐ **이슈 #92 실측 확정**: 사용자의 실제 재실행(Finder 더블클릭·`open`)은 두 번째 프로세스를 만들지 않고 기존 인스턴스로 reopen Apple Event 를 보내므로, 이중 실행은 `open -n` 같은 강제 경로에서만 생긴다. 그 강제 경로의 두 번째 프로세스는 종료 직전에 분산 알림으로 기존 인스턴스에 설정 창을 열게 한다(이슈 #68, §2 시나리오 D).
...
14. **설정 창 닫기 — 닫아도 앱은 계속 돈다 (이슈 #88).** 설정 창을 빨간 버튼·`⌘W` 로 닫아도 앱은 종료되지 않는다 — `ExitRequested`(main.rs) 가 암묵적 종료 요청을 `prevent_exit()` 로 막는다. ⭐ **닫기 → 숨김(창 상주) 세부는 설정 창을 소유하는 F-09(`preferences-ui.md` §3.3) 소관** — 이 문서는 "닫아도 앱은 그대로(엔진은 백그라운드 동작), `Quit` 만이 종료"까지만 정의한다.
...
- [ ] 실행 중인 인스턴스를 Finder 더블클릭·`open` 으로 다시 실행하면, 두 번째 프로세스는 생기지 않고 기존 인스턴스의 **설정 창이 열린다**(이미 열려 있으면 중복 없이 포커스). `open -n` 같은 강제 두 번째 프로세스는 분산 알림으로 같은 결과를 내고 즉시 종료된다. (이슈 #92·#68)
...
- [ ] ⭐ About 창(이슈 #87·#96)은 상단 히어로에 **앱 아이콘**(라이트/다크 자동 대응)·앱 이름·버전을 표시하고, 5개 언어 어느 것에서도 **스크롤바 없이** 본문 전체가 보이는 **540×480pt 고정 크기**(`resizable(false)`)로 열리며, 닫아도 숨김 상주(#88)·언어 변경 즉시 반영 배선이 유지된다.

## 9. 미해결 질문

이번 실측으로 **해소된 항목**(더 이상 §9 에 남지 않음): 메뉴바 메뉴 전체 구성·순서·라벨, `General` 탭의 실제 항목·기본값, 로그인 실행 방식(헬퍼 앱 + `SMLoginItemSetEnabled`/`SMAppService` 이중 경로), `LSUIElement`. ⭐ **이슈 #92 — 이중 실행 시 동작**: "두 번째 프로세스가 생기고 즉시 종료"는 `open -n` 강제 경로에서만이고, 실사용 재실행(Finder·`open`)은 두 번째 프로세스 없이 reopen Apple Event 가 오며 클론은 그 이벤트로 설정 창을 연다(§2 시나리오 D, §8). 원본 관찰(원본 SuperKey 가 Finder 재실행을 어떻게 처리하는지)은 여전히 미관찰로 남는다.
