...
| `About` | F-10 (정보 행 내용은 F-09·F-13 과 공유) | **재현 → 독립 창 확정(이슈 #87).** 원본엔 `About.storyboardc` 존재로 About 창이 있음이 확인됐다(실측: 번들 구조). 클론도 **별도 정보(About) 창**(`ui/about.html`, `show_about_window`, 이슈 #87)으로 재현한다 — ⚠️ 이 문서 4987~4994 행의 옛 결정("별도 About 창을 만들지 않고 설정 창 General 탭에 두었다")은 사용자 피드백(2026-09-02)으로 **뒤집혔다**. About 창은 `plan/issue-87-about-window.md` D1~D6 을 따른다(정보 행 · `Check for Updates…` 버튼(F-13 재사용) · 닫아도 숨김 상주(#88 정책)). |
