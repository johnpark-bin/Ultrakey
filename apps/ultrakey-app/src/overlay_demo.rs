...
//!
//! F-01(세션 상태 머신)이 아직 없어서, 오버레이를 실제로 띄우고 후보를 채우고
//! 키 입력을 흉내 낼 **호출자**가 존재하지 않는다. 이 모듈이 그 자리를
//! 임시로 대신해 `manual-verification.md` 항목 9 를 수행 가능하게 만든다.
//! F-01 이 들어오면 이 모듈은 지워도 된다 — 제품 코드는 `overlay.rs` 쪽이다.
//!
