...
//!
//! 명세 §3.5 는 트라이얼 기산점을 **Keychain**에 저장해 앱 삭제·재설치에도 유지한다.
//! Keychain(`Security.framework`) 접근은 `ultrakey-platform` 의 `unsafe` FFI 책임이고,
...
///
/// ⚠️ 요구사항(명세 §3.5): **앱 삭제·재설치에도 기산점이 살아 있어야 한다**
/// (Keychain). 저장 위치를 결정하는 것은 구현체(어댑터)의 책임이다.
